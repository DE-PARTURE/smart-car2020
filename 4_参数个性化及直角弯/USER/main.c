...
u8 ActionIndex = 0;//0->直角弯检测 1->锐角弯检测
//CCD检测相关
u8 CCD_ChaZhi = 0,CCD_YuzhiBias = 2;
...
u16 ADV[128]={0};              
u8 Bluetooth_Velocity=255,APP_RX;                 //蓝牙遥控速度和APP接收的数据
...
int main(void)
{
	...
	uart3_init(9600); 						//=====串口3初始化超声波
	if(Flag_Way==2)ccd_Init();  			//=====CCD初始化	
	Flash_Read();	                   //=====读取PID参数
	while(1)
		{     
